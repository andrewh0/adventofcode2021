...
const input2 = `cbgefad agc fdega cgdf ecdgfa efgca gaefbd edagbc cg ecafb | cdgafbe cfdg cg gac
...
const numbers = {
  ...
  7: new Set(["a", "c", "f"]),
  4: new Set(["b", "c", "d", "f"]),
  3: new Set(["a", "c", "d", "f", "g"]), // missing b, e
  2: new Set(["a", "c", "d", "e", "g"]), // missing b, f
  5: new Set(["a", "b", "d", "f", "g"]), // missing c, e
  6: new Set(["a", "b", "d", "e", "f", "g"]), // missing c
  0: new Set(["a", "b", "c", "e", "f", "g"]), // missing d
  9: new Set(["a", "b", "c", "d", "f", "g"]), // missing e
  8: new Set(["a", "b", "c", "d", "e", "f", "g"]),
};

function intersection(s1, s2) {
  const s3 = new Set();
  for (let i of Array.from(s1)) {
    if (s2.has(i)) {
      s3.add(i);
    }
  }
...
function getMapping(segments) {
  const numbers = Array(10).fill(null);
  const numberMap = {};
  for (let word of segments) {
    const letters = word.split("");
    const wordSet = new Set(letters);
    const wordAlpha = letters.sort().join("");
    if (word.length === 2) {
      numbers[1] = wordSet;
      numberMap[wordAlpha] = 1;
    } else if (word.length === 3) {
      numbers[7] = wordSet;
      numberMap[wordAlpha] = 7;
    } else if (word.length === 4) {
      numbers[4] = wordSet;
      numberMap[wordAlpha] = 4;
    } else if (word.length === 7) {
      numbers[8] = wordSet;
      numberMap[wordAlpha] = 8;
    }
  }
  ...
  for (let word of segments) {
    ...
    if (word.length === 5) {
      // All of 1's segments are in 3
      if (intersection(numbers[1], wordSet).size === 2) {
        numbers[3] = wordSet;
        numberMap[wordAlpha] = 3;
      }

      // Exactly two of 4's segments are in 2
      else if (intersection(numbers[4], wordSet).size === 2) {
        numbers[2] = wordSet;
        numberMap[wordAlpha] = 2;
      }

      // Must be 5
      else {
        numbers[5] = wordSet;
        numberMap[wordAlpha] = 5;
      }
    }
  }
  ...
  for (let word of segments) {
    ...
    if (word.length === 6) {
      // Segments for 4 are a subset of 9
      if (intersection(numbers[4], wordSet).size === 4) {
        numbers[9] = wordSet;
        numberMap[wordAlpha] = 9;
      }

      // Segments for 1 are a subset of 0
      else if (intersection(numbers[1], wordSet).size === 2) {
        numbers[0] = wordSet;
        numberMap[wordAlpha] = 0;
      }

      // Must be 6
      else {
        numbers[6] = wordSet;
        numberMap[wordAlpha] = 6;
      }
    }
  }
  return numberMap;
...
function parseInput(input) {
  return input.split("\n").map((line) => {
    ...
    return [inputStr.trim().split(" "), outputStr.split(" ")];
  });
}

function solvePart1(input) {
  const lines = parseInput(input);
  return lines.reduce((sum, [_input, output]) => {
    let count = 0;
...
function solvePart2(input) {
  ...
  for (let line of lines) {
    const [segments, output] = line;
    const mapping = getMapping(segments);
    let numStr = "";
    for (let w of output) {
      const wordAlpha = w.split("").sort().join("");
      numStr += `${mapping[wordAlpha]}`;
    }
    sum += parseInt(numStr);
  }
  return sum;
